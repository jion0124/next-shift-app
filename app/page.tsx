...
export default function Home() {
  ...
  const [selectedMode, setSelectedMode] = useState<string>('preferred');

  const handleCellClick = (employee: string, dayKey: string) => {
    setShifts((prevShifts: Shifts) => {
  ...
  const generateShifts = async () => {
    try {
        const response = await fetch('/api/generate_shifts', {
            method: 'POST',
            headers: {
  ...
  const getDaysByType = (classname: string) => {
